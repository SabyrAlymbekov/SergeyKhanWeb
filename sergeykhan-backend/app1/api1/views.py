...
def transfer_order_to_warranty_master(request, order_id):
    """
    Передача заказа гарантийному мастеру
    """
    if request.user.role not in ['super-admin', 'admin', 'curator']:
        return Response({'error': 'Permission denied'}, status=status.HTTP_403_FORBIDDEN)
    ...
    if not warranty_master_id:
        return Response({'error': 'Warranty master ID is required'}, status=status.HTTP_400_BAD_REQUEST)
    
    try:
        warranty_master = CustomUser.objects.get(id=warranty_master_id, role='warrant-master')
    except CustomUser.DoesNotExist:
        return Response({'error': 'Invalid warranty master ID'}, status=status.HTTP_400_BAD_REQUEST)
    ...
    return Response({
        'message': 'Order transferred to warranty master',
        'order_id': order.id,
...
def complete_order(request, order_id):
    """
    Завершение обычного заказа (не гарантийного)
    """
    try:
        order = Order.objects.get(id=order_id)
    except Order.DoesNotExist:
    ...
    try:
        ...
        parts_cost = Decimal(str(request.data.get('parts_cost', 0)))
        transport_cost = Decimal(str(request.data.get('transport_cost', 0)))
        total_received = Decimal(str(request.data.get('total_received', 0)))
        
        # Расчёт чистой прибыли
        net_profit = total_received - parts_cost - transport_cost
        if net_profit < 0:
            return Response({'error': 'Net profit cannot be negative'}, status=400)
        
        # Обновляем заказ
        order.final_cost = total_received
        order.expenses = parts_cost + transport_cost
        order.status = 'завершен'
        order.save()
        
        # Если указано время прибытия клиента, сохраняем его
        client_arrival_datetime = request.data.get('client_arrival_datetime', None)
        if client_arrival_datetime:
            try:
                order.client_arrival_datetime = client_arrival_datetime
                order.save(update_fields=['client_arrival_datetime'])
            except Exception as e:
                # Ошибка при сохранении времени не должна мешать завершению заказа
                print(f"Error saving client arrival time: {e}")
        ...
        log_order_action(
            ...
            action='completed',
            performed_by=request.user,
            description=f'Заказ #{order.id} завершен. Итоговая стоимость: {total_received}₸, Чистая прибыль: {net_profit}₸',
            old_value=f'Статус: {old_status}, Стоимость: {old_final_cost}',
            new_value=f'Статус: завершен, Стоимость: {total_received}₸'
        )
