...
import { Table, TableHeader, TableHead, TableBody, TableRow, TableCell, } from '@workspace/ui/components/table';
...
const AbonentsPage = () => {
    ...
    const [abonentEnums, setAbonentEnums] = useState([]);
    const [selectedAbonent, setSelectedAbonent] = useState(null);
    const [abonentInfo, setAbonentInfo] = useState(null);
    ...
    const handleEnumClick = async (id) => {
    ...
    return (<div className="p-4 flex flex-col md:flex-row gap-8">
      <div className="w-full mb-6 md:mb-0 md:w-1/2">
        <div className="flex items-end gap-2 mb-4">
          <div>
            ...
            <Input type="number" placeholder="Введите ID..." value={searchId} onChange={e => setSearchId(e.target.value)} className="w-40"/>
          </div>
        ...
        {abonentInfo && selectedAbonent === searchId && (<div className="mb-4 p-4 border rounded bg-background">
        ...
        <div className="flex items-end gap-2 mb-4">
          <div>
            ...
            <Input type="number" placeholder="Введите ID..." value={deleteId} onChange={e => setDeleteId(e.target.value)} className="w-40"/>
          </div>
      ...
      <div className="flex-1">
        ...
        <Table>
          ...
          <TableBody>
            {abonents.map((item, idx) => (<TableRow key={idx}>
      ...
      <div className="flex-1">
        ...
        <Table>
          ...
          <TableBody>
            {abonentEnums.map((item, idx) => (<TableRow key={idx} className="cursor-pointer hover:bg-muted" onClick={() => handleEnumClick(item.ID)} style={{ background: selectedAbonent === item.ID ? '#f3f4f6' : undefined }}>
        ...
        {abonentInfo && (<div className="mt-4 p-4 border rounded bg-background">
            <div className="font-semibold mb-2">Информация об абоненте:</div>
            <pre className="text-xs whitespace-pre-wrap">{JSON.stringify(abonentInfo, null, 2)}</pre>
          </div>)}
      </div>
