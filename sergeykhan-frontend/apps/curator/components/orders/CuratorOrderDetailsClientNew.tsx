...
export default function CuratorOrderDetailsClient({ id }: Props) {
  ...
  const fetchWarrantyMasters = async () => {
    ...
    try {
      // Try to fetch warranty masters - adjust endpoint as needed
      let res = await fetch(`${API}/users/warranty-masters/`, {
        headers: { 
          "Content-Type": "application/json", 
          Authorization: `Token ${token}` 
        },
      ...
      // If specific warranty endpoint doesn't exist, use general masters endpoint
      if (!res.ok && res.status === 404) {
        res = await fetch(`${API}/users/masters/`, {
          headers: { 
            "Content-Type": "application/json", 
            Authorization: `Token ${token}` 
          },
      ...
      console.error("Error fetching warranty masters:", e);
    } finally {
