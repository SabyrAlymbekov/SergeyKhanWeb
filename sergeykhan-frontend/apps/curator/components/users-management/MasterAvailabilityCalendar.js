...
const MasterAvailabilityCalendar = ({ masterId: propMasterId, showMasterSelector = true, onAvailabilityChange, }) => {
    const [events, setEvents] = useState([]);
    const [masters, setMasters] = useState([]);
    const [selectedMasterId, setSelectedMasterId] = useState(propMasterId || "");
    ...
    const fetchMasters = useCallback(async () => {
        try {
            ...
            const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/users/masters/`, {
    ...
    const fetchAvailability = useCallback(async () => {
        ...
        try {
            ...
            if (!token)
                throw new Error("No authentication token found");
            const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/masters/${selectedMasterId}/availability/`, {
                headers: { Authorization: `Token ${token}` },
            });
    ...
    const handleEventClick = (clickInfo) => {
        ...
        const availability = events.find(e => e.extendedProps?.availabilityId === availabilityId);
        if (availability) {
    ...
    const createAvailability = async () => {
        try {
            ...
            await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/api/masters/${selectedMasterId}/availability/`, data, {
                headers: { Authorization: `Token ${token}` },
            });
    ...
    const updateAvailability = async () => {
        ...
        try {
            ...
            await axios.put(`${process.env.NEXT_PUBLIC_API_URL}/api/masters/${selectedMasterId}/availability/${selectedAvailability.id}/`, data, {
                headers: { Authorization: `Token ${token}` },
            });
    ...
    const deleteAvailability = async () => {
        ...
        try {
            ...
            if (!token)
                throw new Error("No authentication token found");
            await axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/api/masters/${selectedMasterId}/availability/${selectedAvailability.id}/`, {
                headers: { Authorization: `Token ${token}` },
            });
    ...
    return (<Card>
            <CardHeader>
                <div className="flex items-center justify-between">
                    ...
                    {selectedMaster && (<Badge variant="outline">
                            {selectedMaster.email}
                        </Badge>)}
                </div>
                
                {showMasterSelector && (<div className="flex items-center space-x-4">
                        ...
                        <Select value={selectedMasterId} onValueChange={setSelectedMasterId}>
                            ...
                            <SelectContent>
                                {masters.map((master) => (<SelectItem key={master.id} value={master.id}>
            ...
            <CardContent>
                {error && (<div className="text-red-500 text-center py-4 mb-4">
                        <p>{error}</p>
                ...
                {!selectedMasterId ? (<div className="text-center py-8 text-gray-500">
                        Please select a master to view their availability
                    </div>) : (<div className="calendar-container">
                        <FullCalendar plugins={[dayGridPlugin, timeGridPlugin, interactionPlugin]} initialView="timeGridWeek" events={events} selectable selectMirror dayMaxEvents weekends select={handleDateSelect} eventClick={handleEventClick} headerToolbar={{
                left: "prev,next today",
                center: "title",
                right: "dayGridMonth,timeGridWeek,timeGridDay",
            }} height="auto" slotMinTime="06:00:00" slotMaxTime="22:00:00" allDaySlot={false} loading={setIsLoading}/>
                    </div>)}
                ...
                <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                    <DialogContent>
                        ...
                        {dialogMode !== "delete" && (<div className="grid gap-4 py-4">
                                <div className="grid grid-cols-4 items-center gap-4">
                                    <Label htmlFor="date" className="text-right">
                                        Date
                                    </Label>
                                    <Input id="date" type="date" value={selectedDate} onChange={(e) => setSelectedDate(e.target.value)} className="col-span-3"/>
                                </div>
                                <div className="grid grid-cols-4 items-center gap-4">
                        ...
                        <DialogFooter>
                            {dialogMode === "edit" && (<Button onClick={handleDeleteClick} variant="destructive">
                                    Delete
                                </Button>)}
