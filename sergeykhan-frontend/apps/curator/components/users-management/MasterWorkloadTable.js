...
import { DropdownMenu, DropdownMenuCheckboxItem, DropdownMenuContent, DropdownMenuTrigger, DropdownMenuItem, } from "@workspace/ui/components/dropdown-menu";
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow, } from "@workspace/ui/components/table";
import { Badge } from "@workspace/ui/components/badge";
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@workspace/ui/components/card";
const MasterWorkloadTable = ({ refreshTrigger = 0 }) => {
    const router = useRouter();
    const [sorting, setSorting] = React.useState([]);
    const [columnFilters, setColumnFilters] = React.useState([]);
    const [searchInput, setSearchInput] = React.useState("");
    const [selectedSearchColumn, setSelectedSearchColumn] = React.useState("email");
    ...
    const fetchWorkloadData = React.useCallback(async () => {
        try {
            ...
            const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/masters/workload/all/`, {
                headers: {
    ...
    const columns = React.useMemo(() => [
        ...
        },
        {
            ...
            header: "Name",
            cell: ({ row }) => (<span>{row.original.master.first_name} {row.original.master.last_name}</span>),
        },
        {
            accessorKey: "orders_count",
            header: "Active Orders",
            cell: ({ row }) => (<Badge variant={row.original.orders_count > 5 ? "destructive" : "default"}>
        ...
        },
        {
            ...
            cell: ({ row }) => {
                ...
                return (<Badge variant={getWorkloadColor(percentage)}>
        ...
        },
        {
            ...
            cell: ({ row }) => (<DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                            <DropdownMenuItem onClick={() => router.push(`/workload/${row.original.master.id}`)}>
                                View Details
                            </DropdownMenuItem>
                            <DropdownMenuItem onClick={() => router.push(`/masters/${row.original.master.id}/availability`)}>
                                Manage Availability
                            </DropdownMenuItem>
                        </DropdownMenuContent>
                    </DropdownMenu>),
            ...
            <CardContent>
                <div className="w-full">
                    <div className="flex items-center py-4 space-x-4">
                        <Input placeholder={`Search by ${selectedSearchColumn}...`} value={searchInput} onChange={(event) => setSearchInput(event.target.value)} className="max-w-sm"/>
                        <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                            ...
                            <DropdownMenuContent align="end">
                                <DropdownMenuCheckboxItem checked={selectedSearchColumn === "master.email"} onCheckedChange={() => setSelectedSearchColumn("master.email")}>
                                    Email
                                </DropdownMenuCheckboxItem>
                                <DropdownMenuCheckboxItem checked={selectedSearchColumn === "master.name"} onCheckedChange={() => setSelectedSearchColumn("master.name")}>
                                    Name
                                </DropdownMenuCheckboxItem>
                            </DropdownMenuContent>
                        </DropdownMenu>
