...
import { Search, Edit, Clock, User, AlertCircle } from 'lucide-react';
import axios from 'axios';
import { API } from '@shared/constants/constants';
const columnHelper = createColumnHelper();
const MasterWorkloadTableRussian = () => {
    ...
    const columns = React.useMemo(() => [
        ...
        columnHelper.accessor('total_orders_today', {
            ...
            cell: info => (<Badge variant="default" className="flex items-center gap-1">
        ...
        columnHelper.display({
            ...
            <Button variant="outline" size="sm" onClick={() => editAvailability(row.original.master_id)} className="flex items-center gap-1">
    ...
    const editAvailability = (masterId) => {
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {masterData.length > 0
            ? Math.round(masterData.reduce((sum, master) => sum + master.total_orders_today, 0) / masterData.length)
            : 0} заказов/день
              </div>
            </CardContent>
          </Card>
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center gap-4">
                <div className="relative">
                  ...
                  <Input placeholder="Поиск мастеров..." value={globalFilter} onChange={(e) => setGlobalFilter(e.target.value)} className="pl-10"/>
                </div>
                  ...
                  <table className="w-full">
                    <thead className="bg-muted/50">
                      {table.getHeaderGroups().map(headerGroup => (<tr key={headerGroup.id}>
                          {headerGroup.headers.map(header => (<th key={header.id} className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider cursor-pointer" onClick={header.column.getToggleSortingHandler()}>
                    ...
                    <tbody className="bg-background divide-y divide-border">
                      {table.getRowModel().rows.map(row => (<tr key={row.id} className="hover:bg-muted/50 transition-colors">
