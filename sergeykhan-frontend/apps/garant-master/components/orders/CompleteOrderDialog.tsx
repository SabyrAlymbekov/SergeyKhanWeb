...
interface WarrantyCompletionData {
...
export default function CompleteOrderDialog({ order, onOrderCompleted }: CompleteOrderDialogProps) {
  ...
  const [formData, setFormData] = useState<WarrantyCompletionData>({
    beforePhoto: null,
    afterPhoto: null,
    workDescription: "",
    partsCost: 0,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Use warranty completion endpoint for garant-master
      const response = await fetch(`${API}/orders/${order.id}/complete_warranty_order/`, {
        method: "PATCH",
        headers: {
      ...
      if (!response.ok) {
        throw new Error("Ошибка при завершении гарантийного заказа");
      }
      ...
      console.error("Ошибка при завершении гарантийного заказа:", error);
      alert("Ошибка при завершении гарантийного заказа. Попробуйте еще раз.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Завершение гарантийного заказа #{order.id}</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Textarea
              id="workDescription"
              placeholder="Подробно опишите выполненную гарантийную работу..."
          ...
          <div className="p-4 border rounded-lg">
            <div className="grid grid-cols-3 gap-4 text-sm">
              ...
              <div>
                ...
                <div className="text-green-600">{formData.totalReceived.toFixed(2)} ₸</div>
              </div>
              <div>
