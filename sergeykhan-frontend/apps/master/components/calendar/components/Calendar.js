...
export default function Calendar({ userRole = 'user' }) {
    ...
    return (<div className="calendar-container">
      ...
      <FullCalendar plugins={[dayGridPlugin, timeGridPlugin, interactionPlugin]} initialView="timeGridWeek" events={events} selectable={isMaster} dateClick={handleDateClick} editable={isMaster} droppable={isMaster} select={handleDateSelect} eventDrop={handleEventDrop} eventResize={handleEventResize} eventClick={handleEventClick} height="auto" timeZone="local" nowIndicator={true} eventDisplay="block" dayMaxEvents={false} eventMaxStack={3} headerToolbar={{
        ...
        <DialogContent>
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="flex gap-2">
              <Input type="date" placeholder="Дата начала" value={selectedDate ? new Date(selectedDate).toISOString().slice(0, 10) : ""} onChange={handleStartDateChange} className="w-1/2"/>
              <TimeInput value={startTime} onChange={setStartTime} className="w-1/2 border px-2 py-1 rounded"/>
            </div>
            <div className="flex gap-2">
              <Input type="date" placeholder="Дата окончания" value={selectedDateEnd ? new Date(selectedDateEnd).toISOString().slice(0, 10) : ""} onChange={handleEndDateChange} className="w-1/2"/>
