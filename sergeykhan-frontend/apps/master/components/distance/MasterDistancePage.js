...
const MasterDistancePage = () => {
    ...
    const { loading, error, fetchMasterDistanceWithOrders, takeOrder } = useDistanceApi();
    ...
    const getVisibilityDescription = (hours) => {
        if (hours === 24)
            return "Стандартная видимость: 24 часа";
        if (hours === 28)
            return "Обычная дистанционка: +4 часа (28 часов всего)";
        if (hours === 48)
            return "Суточная дистанционка: +24 часа (48 часов всего)";
            ...
            <Card>
                <CardHeader>
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between">
                        ...
                        <Badge variant={distanceData.distance_info.distance_level === 2 ? "default" :
            distanceData.distance_info.distance_level === 1 ? "secondary" :
                "outline"} className={`text-sm ${distanceData.distance_info.distance_level === 2 ? "bg-green-600 hover:bg-green-700" :
                            ...
                            <CardContent className="pt-4">
                                <div className="space-y-2">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        {distanceData.distance_info.meets_requirements.level_1 && (<Badge variant="outline" className="text-green-600 border-green-200 bg-green-50">
                                                ✓
                                            </Badge>)}
                                    </div>
                            ...
                            <CardContent className="pt-4">
                                <div className="space-y-2">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        {distanceData.distance_info.meets_requirements.level_2 && (<Badge variant="outline" className="text-green-600 border-green-200 bg-green-50">
                                                ✓
                                            </Badge>)}
                                    </div>
                                    <p className="text-xs text-muted-foreground">
            ...
            <Card>
                <CardHeader>
                    <div className="flex items-center justify-between">
                        ...
                        <Button onClick={loadDistanceData} disabled={loading} variant="outline">
                ...
                <CardContent>
                    <div className="space-y-4">
                        {distanceData.orders.length === 0 ? (<div className="text-center py-8 space-y-2">
                            ...
                            </div>) : (distanceData.orders.map((order) => (<Card key={order.id} className="hover:shadow-md transition-shadow">
                                    <CardContent className="pt-4">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                            ...
                                            <div className="flex items-center justify-between pt-2">
                                                <Badge variant={order.status === 'новый' ? 'default' :
                order.status === 'в обработке' ? 'secondary' :
                    'outline'} className={order.status === 'новый' ? 'bg-blue-600 hover:bg-blue-700' :
                order.status === 'в обработке' ? 'bg-yellow-600 hover:bg-yellow-700 text-white' :
                    'text-muted-foreground'}>
                                                    {order.status}
                                                </Badge>
                                                <Button size="sm" disabled={takingOrderId === order.id || loading} onClick={() => handleTakeOrder(order.id)}>
                                                    {takingOrderId === order.id ? (<div className="flex items-center gap-2">
