...
export default function MasterOrderDetailsClient({ id }) {
    ...
    return (<div className="container mx-auto p-6 space-y-6">
        ...
        <Badge variant={order.status === 'новый' ? 'default' :
            order.status === 'в обработке' ? 'secondary' :
                order.status === 'назначен' ? 'outline' :
                    'destructive'} className={`text-sm ${order.status === 'новый' ? 'bg-blue-600 hover:bg-blue-700' :
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          {/* Cost */}
          {order.final_cost && (<div className="flex items-center gap-3 p-4  rounded-lg border border-green-200">
          ...
          {order.assigned_master && (<div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
      ...
      {/* Action Buttons */}
      {isOrderAvailable && (<Card>
          <CardContent className="pt-6">
            <div className="text-center space-y-4">
              <p className="text-lg font-medium">
                Этот заказ доступен для взятия
              </p>
              <Button size="lg" disabled={takingOrder} onClick={handleTakeOrder} className="min-w-[200px]">
                {takingOrder ? (<div className="flex items-center gap-2">
