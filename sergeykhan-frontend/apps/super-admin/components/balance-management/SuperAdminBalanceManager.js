...
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogTrigger, DialogFooter, DialogClose, } from '@workspace/ui/components/dialog';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue, } from '@workspace/ui/components/select';
import { Card, CardContent, CardHeader, CardTitle } from '@workspace/ui/components/card';
import { Badge } from '@workspace/ui/components/badge';
import { Loader2, Plus, Minus, History, Wallet, PiggyBank, Users } from 'lucide-react';
const SuperAdminBalanceManager = ({ userId }) => {
    ...
    // Form state
    const [balanceType, setBalanceType] = useState('current');
    const [actionType, setActionType] = useState('top_up');
    ...
    const fetchBalance = async () => {
        if (!userId)
            return;
        try {
            const response = await axios.get(`${API}/api/balance/${userId}/detailed/`, { headers: { Authorization: `Token ${token}` } });
    ...
    const fetchLogs = async () => {
        if (!userId)
            return;
        try {
            const response = await axios.get(`${API}/api/balance/${userId}/logs/detailed/`, { headers: { Authorization: `Token ${token}` } });
    ...
    const fetchAllBalances = async () => {
        try {
            const response = await axios.get(`${API}/api/balance/all/`, { headers: { Authorization: `Token ${token}` } });
    ...
    const handleBalanceModification = async () => {
        ...
        setProcessing(true);
        try {
            await axios.post(`${API}/api/balance/${userId}/modify/`, {
            ...
            <Card>
                ...
                <CardContent>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
                        {allBalances.slice(0, 6).map((userBalance) => (<Card key={userBalance.user_id} className="p-3">
                    ...
                    <Dialog open={allBalancesDialogOpen} onOpenChange={setAllBalancesDialogOpen}>
                        ...
                        <DialogContent className="sm:max-w-[800px] max-h-[80vh] overflow-y-auto">
                            ...
                            <div className="space-y-3">
                                {allBalances.map((userBalance) => (<Card key={userBalance.user_id}>
                    ...
                    <div className="flex gap-4">
                            ...
                            <DialogContent className="sm:max-w-[500px]">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <Input id="amount" type="number" placeholder="Введите сумму" value={amount} onChange={(e) => setAmount(e.target.value)} min="0" step="0.01"/>
                                    </div>

                                    <div className="space-y-2">
                                        <Label htmlFor="reason">Причина</Label>
                                        <Textarea id="reason" placeholder="Укажите причину изменения баланса" value={reason} onChange={(e) => setReason(e.target.value)} rows={3}/>
                                    </div>
                                </div>
                                <DialogFooter>
                                    <DialogClose asChild>
                        ...
                        <Dialog open={logsDialogOpen} onOpenChange={setLogsDialogOpen}>
                            ...
                            <DialogContent className="sm:max-w-[700px] max-h-[80vh] overflow-y-auto">
                                ...
                                <div className="space-y-3">
                                    {logs.length === 0 ? (<p className="text-center text-muted-foreground py-8">
                                            История операций пуста
                                        </p>) : (logs.map((log) => (<Card key={log.id}>
                                                <CardContent className="p-4">
