...
const BalanceManagement = () => {
    ...
    const handleBalanceUpdate = async () => {
        ...
        try {
            const res = await fetch(`${API}/balance/${user.id}/${action}/`, {
                method: 'POST',
                headers: {
                    Authorization: `Token ${token}`,
                    'Content-Type': 'application/json',
    ...
    return (<div className="p-4">
          ...
          <div className="mb-4 flex items-center space-x-2">
              <Input type="text" placeholder="Введите ID пользователя" value={userId} onChange={(e) => setUserId(e.target.value)}/>
              <Button onClick={handleSearch}>Поиск</Button>
          </div>

          {error && <p className="text-red-500 mb-2">{error}</p>}

          {user && (<div className="border p-4 rounded space-y-2">
          ...
          <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
              <DialogContent>
                  ...
                  <div className="my-4 relative">
                      <Input type="number" placeholder="Новый баланс" value={newBalance} onChange={(e) => setNewBalance(e.target.value)} className="pr-8"/>
                      <span className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500">₸</span>
                  </div>

                  {/* Поле «Сумма изменения» осталось без изменений */}
                  <div className="my-4 flex items-center space-x-2">
                      <Input type="number" placeholder="Сумма изменения" value={delta} onChange={(e) => setDelta(e.target.value)}/>
                      <Button onClick={() => {
            const c = parseFloat(newBalance);
            const d = parseFloat(delta);
            if (!isNaN(c) && !isNaN(d))
                setNewBalance(String(c - d));
        }} variant="outline">
                          Уменьшить
                      </Button>
                      <Button onClick={() => {
