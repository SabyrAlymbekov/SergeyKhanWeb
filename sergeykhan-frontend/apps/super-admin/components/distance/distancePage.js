...
const DistancePage = () => {
    ...
    const { loading, error, fetchDistanceSettings, updateDistanceSettings, getAllMastersDistanceDetailed, forceUpdateAllMastersDistance, setMasterDistanceManually } = useDistanceApi();
    ...
    const handleSetMasterDistance = async (masterId, newLevel) => {
                ...
                <CardContent>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-2">
                            ...
                            <Input type="number" value={averageCheckThreshold} onChange={(e) => setAverageCheckThreshold(Number(e.target.value))} placeholder="65000"/>
                        </div>
                        <div className="space-y-2">
                            <label className="text-sm font-medium">
                ...
                <CardContent>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-2">
                            ...
                            <Input type="number" value={dailyOrderSumThreshold} onChange={(e) => setDailyOrderSumThreshold(Number(e.target.value))} placeholder="350000"/>
                        </div>
                        <div className="space-y-2">
                            <label className="text-sm font-medium">
                                Пороговый чистый вал за 10 дней (₸)
                            </label>
                            <Input type="number" value={netTurnoverThreshold} onChange={(e) => setNetTurnoverThreshold(Number(e.target.value))} placeholder="1500000"/>
                        </div>
                        <div className="space-y-2 md:col-span-2">
                            <label className="text-sm font-medium">
                                Дополнительные часы видимости
                            </label>
                            <Input type="number" value={24} disabled className="bg-muted max-w-xs"/>
                        </div>
                    </div>
                </CardContent>
            </Card>
            ...
            <Card>
                <CardHeader>
                    <div className="flex justify-between items-center">
                        ...
                        <Button onClick={handleForceUpdate} disabled={loading} variant="outline">
                            {loading ? (<div className="flex items-center gap-2">
                                    <div className="h-4 w-4 animate-spin rounded-full border-2 border-current border-t-transparent"/>
                                    Обновление...
                                </div>) : ("Обновить все статусы")}
                        </Button>
                    </div>
                </CardHeader>
                <CardContent>
                ...
                {isLoading ? (<div className="text-center py-8">
                        ...
                        <Table>
                            ...
                            <TableBody>
                                {masters.length === 0 ? (<TableRow>
                                        <TableCell colSpan={6} className="text-center py-8 text-muted-foreground">
                                            Нет мастеров с дистанционкой
                                        </TableCell>
                                    </TableRow>) : (masters.map((master) => (<TableRow key={master.master_id}>
                                            <TableCell className="font-medium">
                                                {master.master_email}
                                            </TableCell>
                                            <TableCell>
                                                <Badge variant={master.distance_level === 2 ? "default" :
                    master.distance_level === 1 ? "secondary" :
                        "outline"} className={master.distance_level === 2 ? "bg-green-600 hover:bg-green-700" :
                        ...
                        "text-muted-foreground"}>
                                            ...
                                            <TableCell className="text-right">
                                                <div className="flex items-center justify-end gap-2">
                                                    ...
                                                    {master.meets_requirements?.standard_distance && (<Badge variant="outline" className="text-green-600 border-green-200 bg-green-50">
                                                            ✓
                                                        </Badge>)}
                                                </div>
                                            </TableCell>
                                            <TableCell className="text-right">
                                                <div className="flex items-center justify-end gap-2">
                                                    ...
                                                    {master.meets_requirements?.daily_distance_by_revenue && (<Badge variant="outline" className="text-green-600 border-green-200 bg-green-50">
                                                            ✓
                                                        </Badge>)}
                                                </div>
                                            </TableCell>
                                            <TableCell className="text-right">
                                                <div className="flex items-center justify-end gap-2">
                                                    ...
                                                    {master.meets_requirements?.daily_distance_by_turnover && (<Badge variant="outline" className="text-green-600 border-green-200 bg-green-50">
                                                            ✓
                                                        </Badge>)}
                                                </div>
                                            </TableCell>
                                            <TableCell>
                                                <div className="flex items-center justify-center gap-2">
                                                    <Button onClick={() => handleSetMasterDistance(master.master_id, 0)} variant="outline" size="sm" disabled={loading || master.distance_level === 0} className="text-xs">
                                                        Сбросить
                                                    </Button>
                                                    <Button onClick={() => handleSetMasterDistance(master.master_id, 1)} variant={master.distance_level === 1 ? "default" : "outline"} size="sm" disabled={loading || master.distance_level === 1} className="text-xs">
                                                        Обычная
                                                    </Button>
                                                    <Button onClick={() => handleSetMasterDistance(master.master_id, 2)} variant={master.distance_level === 2 ? "default" : "outline"} size="sm" disabled={loading || master.distance_level === 2} className="text-xs">
                                                        Суточная
                                                    </Button>
                                                </div>
                                            </TableCell>
