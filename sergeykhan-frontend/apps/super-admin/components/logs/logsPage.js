...
const LogsPage = () => {
    ...
    const getActionBadge = (action) => {
        ...
        return actionConfig[action] || { variant: 'outline' };
    };
    ...
    const getTransactionBadge = (type) => {
        ...
        return typeConfig[type] || { variant: 'outline' };
    };
    ...
    const renderLogDetails = (log) => {
        if (!expandedLogs.has(log.id))
            return null;
        return (<TableRow key={`${log.id}-details`} className="hover:bg-transparent">
                <TableCell colSpan={7} className="bg-muted/30 p-0">
                    <div className="p-4 border-l-4 border-primary/20">
                        <div className="space-y-4">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {log.old_value && (<div className="space-y-2">
                                ...
                                {log.new_value && (<div className="space-y-2">
                                        <h4 className="font-semibold text-sm text-muted-foreground flex items-center gap-2">
                            ...
                            {(!log.old_value && !log.new_value) && (<div className="space-y-2">
                                    <h4 className="font-semibold text-sm text-muted-foreground flex items-center gap-2">
                            ...
                            <div className="flex justify-between items-center pt-3 border-t border-border/50">
                                ...
                                <Button size="sm" variant="outline" onClick={() => openOrderDetails(log.order)} className="h-8 text-xs">
    ...
    const renderOrderLogs = () => (<Card>
            <CardHeader>
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        <div className="w-2 h-2 bg-primary rounded-full"></div>
                        <CardTitle>Логи заказов</CardTitle>
                        {orderLogs.length > 0 && (<Badge variant="secondary">
                                {orderLogs.length}
                            </Badge>)}
                    </div>
                    {expandedLogs.size > 0 && (<Button variant="ghost" size="sm" onClick={collapseAllLogs} className="text-xs">
            ...
            <CardContent>
                {error && (<div className="mb-4 p-4 bg-destructive/10 border border-destructive/20 text-destructive rounded-lg">
                        <div className="flex items-center gap-2">
                ...
                {!loading && orderLogs.length === 0 && !error ? (<div className="text-center py-8">
                ...
                return (<React.Fragment key={log.id}>
                                                <TableRow className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={() => toggleLogExpansion(log.id)}>
                                                    <TableCell>
                                                        <Button variant="ghost" size="sm" onClick={(e) => {
                        e.stopPropagation();
                        toggleLogExpansion(log.id);
                    }} className="h-6 w-6 p-0 transition-transform duration-200" style={{
                        transform: isExpanded ? 'rotate(90deg)' : 'rotate(0deg)'
                                                    ...
                                                    <TableCell>
                                                        <Badge variant="secondary" className="hover:bg-primary hover:text-primary-foreground transition-colors cursor-pointer" onClick={(e) => {
                        e.stopPropagation();
                        openOrderDetails(log.order);
                                                    ...
                                                    <TableCell className="text-muted-foreground">
                                                        {log.performed_by_email || (<span className="italic">Система</span>)}
                                                    </TableCell>
                        ...
                        {orderLogsHasNext && (<div className="mt-6 text-center">
                                <Button onClick={loadMoreOrderLogs} disabled={loading} variant="outline" className="min-w-[120px]">
                                    {loading ? (<>
    ...
    const renderTransactionLogs = () => (<Card>
            <CardHeader>
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        <div className="w-2 h-2 bg-green-500 rounded-full"></div>
                        <CardTitle>Логи транзакций</CardTitle>
                        {transactionLogs.length > 0 && (<Badge variant="secondary">
            ...
            <CardContent>
                ...
                {!loading && transactionLogs.length === 0 && !error ? (<div className="text-center py-8">
                ...
                return (<TableRow key={log.id} className="hover:bg-muted/50 transition-colors">
                                                <TableCell className="font-medium">#{log.id}</TableCell>
                                                <TableCell>
                                                    <Badge variant={badgeConfig.variant}>
                                                ...
                                                <TableCell className="text-muted-foreground">
                                                    {log.user_email || (<span className="italic">Не указан</span>)}
                                                </TableCell>
                                                <TableCell className="text-muted-foreground">
                                                    {log.performed_by_email || (<span className="italic">Система</span>)}
                                                </TableCell>
                        ...
                        {transactionLogsHasNext && (<div className="mt-6 text-center">
                                <Button onClick={loadMoreTransactionLogs} disabled={loading} variant="outline" className="min-w-[120px]">
                                    {loading ? (<>
                                            <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-current mr-2"></div>
                                            Загрузка...
                                        </>) : ('Загрузить еще')}
    ...
    return (<div className="container mx-auto py-8 space-y-8">
            ...
            <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value)}>
                ...
                {/* Loading State */}
                {loading && orderLogs.length === 0 && transactionLogs.length === 0 && (<div className="flex flex-col items-center justify-center py-16 space-y-4">
            ...
            <Dialog open={showOrderModal} onOpenChange={closeOrderModal}>
                <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
                    ...
                    {orderDetailsLoading ? (<div className="flex items-center justify-center py-8">
                            ...
                            <Button onClick={() => selectedOrderId && fetchOrderDetails(selectedOrderId)} variant="outline" size="sm">
                                Попробовать снова
                            </Button>
                            ...
                            <Card>
                                ...
                                <CardContent className="space-y-4">
                                    ...
                                    {orderDetails.master_assigned && (<div>
