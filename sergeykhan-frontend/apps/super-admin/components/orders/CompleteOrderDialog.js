...
export default function CompleteOrderDialog({ order, onOrderCompleted }) {
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input id="beforePhoto" type="file" accept="image/*" onChange={handleFileChange('beforePhoto')}/>
            </div>
            <div className="space-y-2">
              <Label htmlFor="afterPhoto">Фото после ремонта</Label>
              <Input id="afterPhoto" type="file" accept="image/*" onChange={handleFileChange('afterPhoto')}/>
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <Textarea id="workDescription" placeholder="Подробно опишите выполненную работу..." value={formData.workDescription} onChange={(e) => setFormData(prev => ({ ...prev, workDescription: e.target.value }))} rows={4}/>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input id="partsCost" type="number" min="0" step="0.01" value={formData.partsCost} onChange={(e) => setFormData(prev => ({ ...prev, partsCost: parseFloat(e.target.value) || 0 }))}/>
            </div>
            <div className="space-y-2">
              <Label htmlFor="transportCost">Транспортные расходы (₸)</Label>
              <Input id="transportCost" type="number" min="0" step="0.01" value={formData.transportCost} onChange={(e) => setFormData(prev => ({ ...prev, transportCost: parseFloat(e.target.value) || 0 }))}/>
            </div>
          </div>

          <div className="space-y-2">
            <Label htmlFor="totalReceived">Полная сумма полученная за заказ (₸)</Label>
            <Input id="totalReceived" type="number" min="0" step="0.01" value={formData.totalReceived} onChange={(e) => setFormData(prev => ({ ...prev, totalReceived: parseFloat(e.target.value) || 0 }))}/>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input id="clientArrivalDate" type="date" value={formData.clientArrivalDate} onChange={(e) => setFormData(prev => ({ ...prev, clientArrivalDate: e.target.value }))}/>
            </div>
            <div className="space-y-2">
              <Label htmlFor="clientArrivalTime">Время приезда *</Label>
              <Input id="clientArrivalTime" type="time" value={formData.clientArrivalTime} onChange={(e) => setFormData(prev => ({ ...prev, clientArrivalTime: e.target.value }))}/>
            </div>
          </div>
        </div>
