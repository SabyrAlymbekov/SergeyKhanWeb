...
export default function SuperAdminOrderDetailsClient({ id }) {
    ...
    const [warrantyMasters, setWarrantyMasters] = useState([]);
    ...
    const handleRemoveMaster = async () => {
        if (!order || !order.assigned_master)
            return;
        const token = localStorage.getItem("token");
        try {
            ...
            if (res.ok) {
                setOrder({ ...order, assigned_master: null });
    ...
    const handleTransferSubmit = async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                setOrder({ ...order, status: 'передан на гарантию', transferred_to: warrantyMasterIdNum });
    ...
    if (loading)
        return <div className="p-4 text-center">Загрузка заказа…</div>;
    if (error)
    ...
    return (<div className="p-6 space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          {/* Cost */}
          {order.final_cost && (<div className="flex items-center gap-3 p-4 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-800">
          ...
          {order.assigned_master && (<div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
      ...
      <div className="mt-8 space-y-4">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <Dialog open={isAssignOpen} onOpenChange={(open) => {
            ...
            <DialogContent>
              ...
              <DialogFooter>
                ...
                <Button onClick={handleAssignSubmit} disabled={!newMasterId || mastersLoading}>
                  Назначить
                </Button>
              </DialogFooter>
            </DialogContent>
            ...
            <DialogContent>
              ...
              <div className="space-y-4">
                ...
                <Select value={warrantyMasterId} onValueChange={setWarrantyMasterId} disabled={transferLoading}>
                  ...
                  <SelectContent>
                    {warrantyMasters.length === 0 && !warrantyMastersLoading ? (<SelectItem value="no-masters" disabled>
                        Гарантийные мастера не найдены
                      </SelectItem>) : (warrantyMasters.map((master) => (<SelectItem key={master.id} value={master.id.toString()}>
                          {master.full_name} ({master.email}) - ID: {master.id}
                        </SelectItem>)))}
                  </SelectContent>
                </Select>
              ...
              <DialogFooter>
                ...
                <Button variant="destructive" onClick={handleTransferSubmit} disabled={!warrantyMasterId || warrantyMastersLoading || transferLoading || warrantyMasters.length === 0}>
          ...
          {/* Remove Master */}
          {order.assigned_master && (<Button variant="outline" onClick={handleRemoveMaster} className="flex items-center gap-2">
              <UserX className="h-4 w-4"/>
              Снять мастера
            </Button>)}

          {/* Complete Order */}
          {order.status !== 'завершен' && (<CompleteOrderDialog order={order} onOrderCompleted={handleOrderCompleted}/>)}
      ...
      {order.status === 'завершен' && (<div className="mt-8 p-6 bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-800">
