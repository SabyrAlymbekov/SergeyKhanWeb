...
const UsersTab = () => {
    ...
    const mastersData = React.useMemo(() => staffData.filter((user) => user.role === "master"), [staffData]);
    const curatorsData = React.useMemo(() => staffData.filter((user) => user.role === "curator"), [staffData]);
    const operatorsData = React.useMemo(() => staffData.filter((user) => user.role === "operator"), [staffData]);
    // Доступные вкладки (роли)
    const availableTabs = ["curator", "master", "operator"];
    const [usersType, setUsersType] = useState(availableTabs[0]);
    ...
    return (<ContentLayout title={<div className="flex flex-row gap-3">
                    {availableTabs.map((tab) => (<Button key={tab} variant={usersType === tab ? "default" : "outline"} onClick={() => handleClick(tab)}>
                            {tab === "master"
                    ? "Masters"
                    : tab === "curator"
                        ? "Curators"
                ...
                </div>} bg={typeof window !== "undefined" && window.innerWidth < 768
