...
export function MasterCalendar({ masterId, userRole = 'user', readOnly = false, showCreateButton = true, apiBaseUrl = "http://127.0.0.1:8000" }) {
    ...
    const handleDateSelect = useCallback((arg) => {
        if (!canEdit)
            return;
    ...
    const createEvent = useCallback(async (eventData) => {
        ...
        try {
            ...
            if (response.status === 200 || response.status === 201) {
                ...
                setEvents(prev => prev.map(event => event.id === tempId ? newEvent : event));
            }
            else {
    ...
    return (<div className="master-calendar-container">
      {canEdit && showCreateButton && (<div className="flex justify-end mb-4">
          <Button onClick={handleNewEventClick}>
            Новое событие
          </Button>
        </div>)}
      
      {readOnly && (<div className="mb-4 p-2 bg-blue-50 dark:bg-blue-900/20 rounded-md text-sm text-blue-700 dark:text-blue-300">
      ...
      <FullCalendar plugins={[dayGridPlugin, timeGridPlugin, interactionPlugin]} initialView="timeGridWeek" events={events} selectable={canEdit} dateClick={canEdit ? handleDateClick : undefined} editable={canEdit} droppable={canEdit} select={canEdit ? handleDateSelect : undefined} eventDrop={canEdit ? handleEventDrop : undefined} eventResize={canEdit ? handleEventResize : undefined} eventClick={canEdit ? handleEventClick : undefined} height="auto" timeZone="local" nowIndicator={true} eventDisplay="block" dayMaxEvents={false} eventMaxStack={3} headerToolbar={{
            ...
            minute: '2-digit',
            hour12: false
        }} slotMinTime="06:00:00" slotMaxTime="23:00:00" allDaySlot={false} slotDuration="00:30:00" slotLabelInterval="01:00:00"/>

      {/* Диалог создания события - только для редактируемого календаря */}
      {canEdit && (<Dialog open={isDialogOpen} onOpenChange={handleDialogClose}>
          <DialogContent>
            <DialogHeader>
            ...
            <div className="space-y-4 py-2">
              <Input type="text" placeholder="Название события" value={newEventTitle} onChange={(e) => setNewEventTitle(e.target.value)} className="w-full"/>
              <div className="flex gap-2">
                <Input type="date" placeholder="Дата начала" value={selectedDate ? new Date(selectedDate).toISOString().slice(0, 10) : ""} onChange={(e) => {
                ...
                <TimeInput value={startTime} onChange={setStartTime} className="w-1/2 border px-2 py-1 rounded"/>
              </div>
              <div className="flex gap-2">
                <Input type="date" placeholder="Дата окончания" value={selectedDateEnd ? new Date(selectedDateEnd).toISOString().slice(0, 10) : ""} onChange={(e) => {
                ...
                <TimeInput value={endTime} onChange={setEndTime} className="w-1/2 border px-2 py-1 rounded"/>
              </div>
            </div>
...
function TimeInput({ value, onChange, ...props }) {
    ...
    return (<input type="text" placeholder="ЧЧ:ММ (24ч)" value={value} onChange={handleChange} onBlur={handleBlur} maxLength={5} {...props}/>);
}
