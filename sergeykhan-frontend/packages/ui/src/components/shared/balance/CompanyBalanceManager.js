...
export const CompanyBalanceManager = ({ userId, showControls = true, readonly = false, }) => {
    ...
    const fetchBalance = async () => {
        if (!userId || !token)
            return;
        try {
            const response = await axios.get(`${API}/api/balance/${userId}/dashboard/`, { headers: { Authorization: `Token ${token}` } });
    ...
    const fetchLogs = async () => {
        if (!token)
            return;
        try {
            const response = await axios.get(`${API}/api/company-balance/logs/`, { headers: { Authorization: `Token ${token}` } });
    ...
    const handleBalanceModification = async () => {
        ...
        setProcessing(true);
        try {
            await axios.post(`${API}/api/company-balance/modify/`, {
    ...
    return (<div className="space-y-6">
      ...
      {showControls && !readonly && (<div className="flex gap-4">
            ...
            <DialogContent className="sm:max-w-[500px]">
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  <select id="action-type" className="w-full border rounded p-2" value={actionType} onChange={(e) => setActionType(e.target.value)}>
