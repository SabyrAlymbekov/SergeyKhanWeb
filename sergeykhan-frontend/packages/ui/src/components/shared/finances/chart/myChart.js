...
export function LogsChart({ visible }) {
    ...
    return (<Card>
      <CardHeader className="flex flex-col sm:flex-row border-b p-0">
        ...
        <div className="flex divide-x">
          {["week", "month"].map(p => (<button key={p} onClick={() => setPeriod(p)} className={`px-6 py-4 flex-1 text-center ${period === p ? "bg-muted/50" : ""}`}>
              {p === "week" ? "Неделя" : "Месяц"}
            </button>))}
        </div>
      </CardHeader>
      ...
      <CardContent className="p-6">
        {data.length === 0 ? (<div className="h-[250px] flex items-center justify-center text-gray-500">
            Логи отсутствуют
          </div>) : (<ChartContainer className="h-[250px]" config={{ count: { label: "Операций" } }}>
            <BarChart data={data} margin={{ left: 12, right: 12 }}>
              <CartesianGrid vertical={false}/>
              <XAxis dataKey="date" axisLine={false} tickLine={false} tick={{ fontSize: 12 }}/>
              <ChartTooltip content={<ChartTooltipContent nameKey="Операций" labelFormatter={val => `Дата: ${val}`}/>}/>
