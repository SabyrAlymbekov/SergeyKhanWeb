...
export function TableOfCalls() {
    ...
    const [calledContacts, setCalledContacts] = React.useState([]);
    ...
    const fetchContacts = React.useCallback(async () => {
        ...
        try {
            const [uncalledRes, calledRes] = await Promise.all([
                api.get('/contacts/uncalled/'),
                api.get('/contacts/called/'),
            ]);
    ...
    const columns = [
        ...
        },
        {
            ...
            header: ({ column }) => {
                ...
                return (<div className="flex items-center cursor-pointer select-none w-32" onClick={() => column.toggleSorting(current !== 'asc')}>
              <span className="flex-1">Date</span>
              <span className="ml-1 w-4 h-4 flex items-center justify-center">
              {current === 'asc' ? (<ArrowUp className="h-4 w-4"/>) : current === 'desc' ? (<ArrowDown className="h-4 w-4"/>) : (<ArrowUp className="h-4 w-4 opacity-0"/>)}
            </span>
            </div>);
            },
        ...
        },
        {
            id: 'actions',
            header: 'Actions',
            cell: ({ row }) => (<Button variant="outline" size="sm" onClick={() => markAsCalled(row.original.id)}>
            Прозвонён
          </Button>),
        },
    ];
    ...
    return (<div className="w-full">
        <div className="flex flex-col gap-5">
          ...
          <div className="flex items-center py-4">
            <Input placeholder="Filter names..." value={table.getColumn('name')?.getFilterValue() ?? ''} onChange={e => table.getColumn('name')?.setFilterValue(e.target.value)} className="max-w-sm"/>
          </div>
                ...
                <AccordionContent>
                  <div className="overflow-hidden">
                    <Table>
                      ...
                      <TableBody>
                        {calledContacts.map(contact => (<TableRow key={contact.id}>
