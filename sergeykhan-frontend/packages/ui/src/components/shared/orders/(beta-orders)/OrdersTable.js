...
import { useReactTable, getCoreRowModel, getFilteredRowModel, getSortedRowModel, getPaginationRowModel, getGroupedRowModel, flexRender, } from "@tanstack/react-table";
import { Checkbox } from "@workspace/ui/components/checkbox";
import { Button } from "@workspace/ui/components/button";
import { Input } from "@workspace/ui/components/input";
import { Dialog, DialogTrigger, DialogContent, DialogHeader, DialogTitle, DialogFooter, DialogClose, } from "@workspace/ui/components/dialog";
...
export function OrdersDataTable({ data, columns, status, isEdit = false, onSelectedChange = () => { }, masterId, isModel = true, }) {
    ...
    const [localData, setLocalData] = useState(() => isModel ? [] : data);
    ...
    const fetchAssignedOrders = async () => {
        if (!masterId)
            return;
        try {
            const res = await axios.get(`${API}/orders/master/${masterId}/`, {
    ...
    const handleRemoveOrder = async (orderId) => {
        try {
            await axios.patch(`${API}/assign/${orderId}/remove/`, {}, { headers: { Authorization: `Token ${token}` } });
            if (isModel)
                fetchAssignedOrders();
            else
    ...
    const handleAssign = async () => {
        try {
            await Promise.all(toAssign.map(order => axios.patch(`${API}/assign/${order.id}/`, { assigned_master: masterId }, { headers: { Authorization: `Token ${token}` } })));
            setToAssign([]);
    ...
    const handleCheckboxChange = (order, checked) => {
    ...
    return (<div className="w-full">
        ...
        <div className="flex items-center py-4">
          <Input placeholder="Найти заказы..." value={globalFilter} onChange={e => setGlobalFilter(e.target.value)} className="max-w-sm"/>
          {status === "curator" && isModel && (<div className="flex gap-2 ml-4">
                <Button variant={isDeleteMode ? "destructive" : "outline"} size="icon" onClick={() => setIsDeleteMode(p => !p)}>
                  <Minus />
                </Button>
          ...
          <div className="ml-auto">
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                {table.getAllColumns().map(col => (<DropdownMenuCheckboxItem key={col.id} checked={col.getIsVisible()} onCheckedChange={v => col.toggleVisibility(!!v)}>{col.id}</DropdownMenuCheckboxItem>))}
              </DropdownMenuContent>
            </DropdownMenu>
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <div className="min-w-[600px] rounded-md border">
            ...
            return (<TableRow key={row.id} className={`${isEdit || isDeleteMode ? "cursor-default" : "cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-800"}`} onClick={() => !isEdit && !isDeleteMode && router.push(`/orders/${order.id}`)}>
                        {/* ID cell */}
                        <TableCell className="text-center font-mono">{order.id}</TableCell>
                        {isEdit && (<TableCell className="text-center">
                              <Checkbox checked={selectedOrders.some(o => o.id === order.id)} onCheckedChange={c => handleCheckboxChange(order, !!c)}/>
                            </TableCell>)}
                        {row.getVisibleCells().map(cell => (<TableCell key={cell.id}>
                              {flexRender(cell.column.columnDef.cell, cell.getContext())}
                            </TableCell>))}
                        {isDeleteMode && (<TableCell className="text-center">
