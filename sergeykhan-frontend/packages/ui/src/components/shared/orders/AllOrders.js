...
export default function ActiveOrders({ isActiveEdit, onSelectedChange, masterId }) {
    ...
    if (loading)
        return <div className="p-4 text-center">Загрузка активных заказов...</div>;
    if (error)
        return <div className="p-4 text-center text-red-500">Ошибка: {error}</div>;
    if (!memoData.length)
        return <div className="p-4 text-center">Активных заказов нет</div>;
    return (<div className="p-4">
        <OrdersDataTable data={memoData} columns={memoCols} isEdit={isActiveEdit} onSelectedChange={onSelectedChange} status="curator" masterId={masterId?.toString()} isModel={false}/>
      </div>);
}
