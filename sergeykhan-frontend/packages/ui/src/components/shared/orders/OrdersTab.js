...
const OrdersTab = ({ status, accessStatus = 'none' }) => {
    ...
    const getAccessTab = () => accessStatus === 'pro' ? '4hours' : '24hours';
    const defaultTab = getAccessTab();
    const availableTabs = status === 'curator'
        ? ['all', defaultTab, 'non-active', 'active']
        : [defaultTab];
    const [ordersType, setOrdersType] = React.useState(defaultTab);
    const handleClick = (type) => setOrdersType(type);
    const renderContent = () => {
        switch (ordersType) {
            case 'all':
    ...
    return (<ContentLayout title={<div className="flex flex-row gap-3">
                    {availableTabs.map((tab) => (<Button key={tab} variant={ordersType === tab ? "default" : "outline"} onClick={() => handleClick(tab)}>
                            {tab === '4hours'
                    ? "4 Hours"
                    : tab === '24hours'
                        ? "24 Hours"
