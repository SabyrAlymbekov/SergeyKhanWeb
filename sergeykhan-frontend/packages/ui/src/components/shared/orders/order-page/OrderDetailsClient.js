...
export default function OrderDetailsClient({ id }) {
    ...
    const handleTransferSubmit = async () => {
        ...
        try {
            ...
            setOrder({ ...order, status: 'назначен' });
    ...
    const getDisplayPhone = () => {
        if (userRole === 'admin' || userRole === 'curator' || userRole === 'super-admin') {
            ...
            if (isAssignedToCurrentUser) {
                ...
                return (<span className="flex items-center gap-2 text-muted-foreground italic">
    ...
    return (<div className="p-4">
      ...
      <Table>
        ...
        <TableBody>
          {rows.map(({ label, value }) => (<TableRow key={label}>
      ...
      {(userRole === 'admin' || userRole === 'curator' || userRole === 'super-admin') && (<div className="mt-6 flex flex-wrap gap-2">
          ...
          <Button onClick={handleRemoveMaster} disabled={!order.assigned_master}>
            Убрать мастера
          </Button>
          ...
          <Dialog open={isTransferOpen} onOpenChange={(open) => {
            ...
            <DialogContent>
              ...
              <Select value={warrantyMasterId} onValueChange={setWarrantyMasterId}>
                ...
                <SelectContent>
                  {warrantyMasters.map((master) => (<SelectItem key={master.id} value={master.id.toString()}>
                      {master.full_name} ({master.email}) - ID: {master.id}
                    </SelectItem>))}
                </SelectContent>
              </Select>
              <DialogFooter>
                ...
                <Button variant="destructive" onClick={handleTransferSubmit} disabled={!warrantyMasterId || warrantyMastersLoading}>
                  Передать
                </Button>
              </DialogFooter>
            </DialogContent>
